...
public class ZEDSpatialMapping
{
    ...
    public bool LoadMesh(string meshFilePath = "ZEDMesh.obj")
    {
        ...
        if (zedCamera.EnableTracking(ref quat, ref tr, true, false, false, false, true, -1.0f, true, sl.POSTIONAL_TRACKING_MODE.GEN_1, System.IO.File.Exists(basePath + ".area") ? basePath + ".area" : "") != sl.ERROR_CODE.SUCCESS)
        {
    ...
    private void SaveMeshNow(string meshFilePath = "Assets/ZEDMesh.obj")
    {
        ...
        lock (lockScanning)
        {
            ...
            if (isTextured)
                objstring.Append("mtllib ").Append(basefilename).Append(".mtl\n");

            foreach (Vector3 vec in vertices)
            {
                //X is flipped because of Unity's handedness.
                objstring.AppendFormat("v {0} {1} {2}\n", -vec.x, vec.y, vec.z);
            }
            objstring.Append("\n");
            foreach (Vector2 uv in uvs)
            {
                objstring.AppendFormat("vt {0} {1}\n", uv.x, uv.y);
            }
            objstring.Append("\n");
            foreach (Vector3 norm in normals)
            {
                objstring.AppendFormat("vn {0} {1} {2}\n", -norm.x, norm.y, norm.z);
            }
            //objstring.Append("\n");
            ...
            for (int i = 0; i < triangles.Length; i += 3)
            {
                ...
                objstring.AppendFormat("f {0}/{0}/{0} {1}/{1}/{1} {2}/{2}/{2}\n",
            ...
            if (isTextured)
            {
                ...
                mtlstring.Append("map_Kd ").Append(basefilename).Append("_material0000_map_Kd.png");
    ...
    private class ZEDSpatialMappingHelper
    {
        ...
        public ZEDSpatialMapping.Chunk CreateNewMesh(int i, Material meshMat, Transform holder)
        {
            ...
            chunk.o.transform.SetLocalPositionAndRotation(Vector3.zero, Quaternion.identity);
#else
