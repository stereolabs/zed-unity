...
bool Unity_IsNan_float4(float3 In)
{
...
half4 computeLightingLWRP(float3 albedo, float3 normals, float3 worldPos, float alpha, float zedfactoraffectreal)
{
	...
	c.rgb = lerp(c.rgb, c.rgb *= mainLight.shadowAttenuation, step(0, dirlightnormdot)); 
	if (!Unity_IsNan_float4(normals)) c.rgb = lerp(c.rgb, c.rgb *= mainLight.shadowAttenuation, step(0, dirlightnormdot));
